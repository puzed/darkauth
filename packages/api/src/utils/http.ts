...
/**
 * Set secure session cookie with __Host- prefix as specified in CORE.md
...
export function setSessionCookie(
  ...
  const base: {
    ...
    path: string;
    maxAge?: number;
  } = { httpOnly: true, secure: true, sameSite: options.sameSite || "Lax", path: "/" };
  if (options.maxAge !== undefined) base.maxAge = options.maxAge;
  setCookie(response, "__Host-DarkAuth", value, base);
}
