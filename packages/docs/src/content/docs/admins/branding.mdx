...
OAuth clients can show an icon on the user dashboard. Uploaded dashboard icons are restricted to passive image formats: PNG, JPEG, WebP, and GIF. DarkAuth normalizes the MIME type, validates image magic bytes, rejects SVG, HTML, and other active content, and serves accepted icons with `X-Content-Type-Options: nosniff`.

If an older stored upload no longer passes the safety checks, the icon endpoint returns 404 instead of serving it from a trusted DarkAuth origin.
