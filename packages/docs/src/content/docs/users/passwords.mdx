...
If an application stores user data under zero-knowledge key material, the user may need an additional recovery flow after signing in with the new password. If the old password material is not available and the application has no other recovery method, older encrypted content may remain unavailable.
