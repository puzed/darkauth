...
function parseOptions() {
  ...
  try {
    return { issuer, overrides: JSON.parse(decodeBase64Url(raw)) } as const;
  } catch {
    try {
...
function getBrandColor(
  ...
  const source = dark ? colorsDark : colors;
  return source?.brandColor || (dark ? "#aec1e0" : "#6600cc");
}

function normalizeBrandingForPreview(branding: BrandingConfig) {
  const fallbackLogo = toDataSvg(
    getDefaultLogoSvg(getBrandColor(branding.colors, branding.colorsDark, false))
  );
  const fallbackLogoDark = toDataSvg(
    getDefaultLogoSvg(getBrandColor(branding.colors, branding.colorsDark, true))
  );
  const normalizeLogo = (value: string | null | undefined, fallback: string) => {
    if (!value) return fallback;
    if (value.startsWith("/api/branding/logo")) return fallback;
    return value;
  };
  return {
    ...branding,
    logoUrl: normalizeLogo(branding.logoUrl, fallbackLogo),
    logoUrlDark: normalizeLogo(branding.logoUrlDark, fallbackLogoDark),
    customCssUrl: null,
  };
}

function applyBrandingDefaults(_issuer: string | null) {
  const w = window as ThemedWindow;
  const cfg: AppConfig = w.__APP_CONFIG__ || {};
  const current: BrandingConfig = cfg.branding || {};
  w.__APP_CONFIG__ = {
    ...cfg,
    branding: normalizeBrandingForPreview(current),
  };
}

function applyBranding(overrides: Overrides, issuer: string | null) {
  applyBrandingDefaults(issuer);
  if (!overrides) return;
  const w = window as ThemedWindow;
  const cfg: AppConfig = w.__APP_CONFIG__ || {};
  const branding = normalizeBrandingForPreview({
    ...(cfg.branding || {}),
    ...(overrides.branding || {}),
  });
  w.__APP_CONFIG__ = { ...cfg, branding };
  if (branding.customCSS !== undefined || branding.customCssUrl !== undefined) {
    const existingStyle = document.getElementById("da-custom-css");
    if (existingStyle) existingStyle.parentElement?.removeChild(existingStyle);
  }
  const existingSheet = document.getElementById("da-branding-css");
  if (existingSheet) existingSheet.parentElement?.removeChild(existingSheet);
}

function applyThemeBehavior(overrides: Overrides) {
  if (!overrides) return;
  const w = window as ThemedWindow;
  const mode =
...
localStorage.getItem = ((key: string) => {
  if (key === "daTheme") return null;
  return originalGetItem(key);
}) as typeof localStorage.getItem;
localStorage.setItem = ((key: string, value: string) => {
  if (key === "daTheme") {
    if (value === "light" || value === "dark") {
      document.documentElement.setAttribute("data-da-theme", value);
      try {
        window.parent?.postMessage(
          { type: "da:theme-changed", theme: value },
...
if (initialOverrides) {
  applyThemeBehavior(initialOverrides);
...
window.addEventListener("message", (e: MessageEvent) => {
  ...
  if (!isBrandingMessage(data)) return;
  overrides = {
    ...(overrides || {}),
    ...data.payload,
    branding: {
  ...
  applyBranding(overrides, issuer);
  revision += 1;
  render();
});
