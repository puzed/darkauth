...
function createMocks(vector: Vector, cfg: Config, isFake: boolean) {
    const originals = {
        ...
        randomBlinder: OPRFClient.prototype.randomBlinder,
        generateDHKeyPair: AKE3DH.prototype.generateDHKeyPair,
    }

    const seq: Uint8Array[] = []
    if (!isFake) {
        seq.push(notNullHex(vector.intermediates.client_public_key))
        seq.push(notNullHex(vector.intermediates.masking_key))
    } else {
        AKE3DH.prototype.generateDHKeyPair = () =>
            Promise.resolve({
                private_key: notNullHex(vector.inputs.client_private_key),
                public_key: notNullHex(vector.inputs.client_public_key),
            })
        seq.push(notNullHex(vector.inputs.masking_key))
    }

    let rbCalls = 0
    OPRFClient.prototype.randomBlinder = function () {
        rbCalls += 1
        if (!isFake && rbCalls === 1) {
    ...
    if (!isFake) {
        seq.push(notNullHex(vector.inputs.envelope_nonce))
    }

    seq.push(notNullHex(vector.inputs.client_nonce))
    seq.push(notNullHex(vector.inputs.client_keyshare_seed))
    seq.push(notNullHex(vector.inputs.masking_nonce))
    seq.push(notNullHex(vector.inputs.server_nonce))
    seq.push(notNullHex(vector.inputs.server_keyshare_seed))

    globalThis.crypto.getRandomValues = function (arr: any) {
        const v = seq.shift() ?? new Uint8Array(0)
        if (arr && typeof arr.set === 'function') {
            const n = Math.min(arr.length ?? 0, v.length)
            arr.set(v.subarray(0, n))
            return arr
        }
        return v as any
    }
    ...
    return function restore() {
        globalThis.crypto.getRandomValues = originals.getRandomValues
        OPRFClient.prototype.randomBlinder = originals.randomBlinder
        AKE3DH.prototype.generateDHKeyPair = originals.generateDHKeyPair
    }
}
...
const vectors = read_test_vectors()
for (let i = 0; i < vectors.length; i++) {
    const vector = vectors[i]
    const opaqueID = vector.config.OPRF
    const res = OpaqueConfig.fromString(opaqueID)
    const run = isOk(res) ? describe : describe.skip

    run(`test-vector-${i}:${opaqueID}`, () => {
        ...
        before(() => {
            ...
            restore = createMocks(vector, cfg, isFake)
