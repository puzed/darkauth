...
- `account_keys` stores account root key metadata and ciphertext.
- `key_envelopes` stores encrypted account-key envelopes.
- `recovery_keys` stores recovery-key verifier metadata.
- `trusted_devices` stores trusted-device envelope metadata.
- `wrapped_root_keys` stores legacy wrapped DRK state for migration compatibility.
