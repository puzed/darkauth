...
Start with [Security Model](/security/model/) for trust boundaries, then read [OPAQUE Security](/security/opaque/) and [DRK Security](/security/drk/) for the parts that make DarkAuth different from a conventional auth server.
