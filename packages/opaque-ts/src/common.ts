...
export class OPRFBaseMode implements OPRFFn {
    ...
    async evaluate(key: Uint8Array, blinded: Uint8Array): Promise<Uint8Array> {
        ...
        const group = Oprf.getGroup(this.id)
        const blindedElt = group.desElt(blinded)
    ...
    async finalize(
        ...
        const group = Oprf.getGroup(this.id)
        const evaluatedElt = group.desElt(evaluationBytes)
        const evaluation = new Evaluation(Oprf.Mode.OPRF, [evaluatedElt])
