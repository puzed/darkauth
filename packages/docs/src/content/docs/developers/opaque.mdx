...
After successful OPAQUE authentication, the client receives an export key. DarkAuth uses this client-only value to unlock the password envelope in the user's keybag. That envelope can unwrap the account root key locally, and current ZK clients derive client app keys from that account material instead of receiving the account root key directly.
