  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (withoutKeys.length > 0) {
        console.warn("[ShareModal] some users missing public_key_jwk", { subs: withoutKeys.map(u=>u.sub) });
      }
      ...
      const results = await Promise.allSettled(
        withKeys.map(async (u) => {
          ...
          const pub = await importJWK(u.public_key_jwk as any, "ECDH-ES");
      ...
      if (successes.length > 0) {
        console.debug("[ShareModal] share completed", { successes: successes.length, failures: failures.length });
        onClose();
      } else {
      ...
      <div className="relative bg-white dark:bg-dark-800 rounded-xl shadow-xl w-full max-w-lg p-5">
        ...
        {selectedList.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-2">
            {selectedList.map((u) => (
              <div key={u.sub} className="px-2 py-1 bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-md text-sm flex items-center gap-2">
                <span>{u.display_name || u.sub}</span>
                <button onClick={() => toggleSelect(u)} className="hover:opacity-70">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-sm">
                        <div className="text-gray-900 dark:text-white">{u.display_name || u.sub}</div>
                        <div className="text-gray-500 dark:text-gray-400 text-xs">{u.sub}</div>
                      </div>
                    </div>
                    <div className={`w-5 h-5 rounded border flex items-center justify-center ${isSel ? "bg-primary-600 border-primary-600" : "border-gray-300 dark:border-dark-600"}`}>
