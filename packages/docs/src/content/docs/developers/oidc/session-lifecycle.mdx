...
Refresh tokens are client-bound. A public client can redeem refresh tokens issued to that public client, and a confidential client must authenticate before redeeming refresh tokens issued to it. A refresh token issued to one client cannot be replayed through another refresh-capable client.

In the default first-party browser profile used by the TypeScript SDK, refresh can rely on HttpOnly cookies instead of JavaScript-readable refresh tokens. That cookie-backed flow is intended for the first-party public client, reducing browser storage exposure.
...
For zero-knowledge apps, logout should clear token state, CAK, legacy DRK material, derived keys, callback state, PKCE verifier, and any ephemeral private key material.
