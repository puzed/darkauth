...
DarkAuth is a self-hosted authentication system for teams that want standards-based login without giving up control of password handling, key custody, or deployment. It behaves like a normal OAuth 2.0 and OpenID Connect provider for applications, while adding OPAQUE password authentication and optional zero-knowledge client app key delivery for clients that need it.
...
<div class="path-grid">
  ...
  <a class="path-card" href="/developers/">
    ...
    <p>Integrate OIDC, PKCE, the TypeScript SDK, ZK key delivery, user APIs, and org/RBAC claims.</p>
  </a>
...
- A zero-knowledge client app key handoff for trusted browser apps using fragment JWE.
