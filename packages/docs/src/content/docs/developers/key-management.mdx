...
The DarkAuth user UI unlocks the ARK once per sign-in. After any unlock it stores an AES-GCM encrypted copy of the ARK in `localStorage` under `DarkAuth_session_ark:<sub>`. The key for that copy comes from `POST /crypto/session-unlock-key`, lives only in the server-side session for that sign-in, and is deleted with `DELETE /crypto/session-unlock-key` or when the sign-in ends. New tabs and reloads restore the ARK without a prompt.

Sign-out, expiry, revoking the sign-in, password reset, and SCIM deactivation destroy the server key. A new sign-in gets a new key. Any restore failure deletes the stored copy and falls back to the normal unlock methods.

The trade-off: a copied browser profile or same-origin script can restore the ARK while the sign-in is live. The unlock policy reports this as `allow_session_unlock`, and SCIM-managed users follow `users.scim.allow_session_unlock` (default `true`).

Relying-party custody is unchanged. Apps still receive only a CAK per authorization and should keep it in memory.
...
Frontend code that handles unlocked keys is inside the trust boundary. Keep key material in memory unless the product deliberately accepts persistent local custody, as the DarkAuth user UI does with session unlock. Clear it on logout, and never log envelopes, plaintext keys, recovery secrets, or fragment JWE payloads.
