...
- Plaintext passwords.
- OPAQUE export keys.
- Plaintext account root keys.
- Plaintext client app keys.
- Plaintext app data protected by client-held keys.
...
If trusted frontend code is malicious, it can read secrets while they are in use. If an app receives a CAK or legacy DRK, that app can misuse it. These are explicit parts of the hosted-web trust boundary.
